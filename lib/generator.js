...
var cdAppPath = global.imagemagickPath ? global.imagemagickPath : '';
...
exports.trimImages = function (files, options, callback) {
	...
	async.eachSeries(files, function (file, next) {
		...
		cdAppPath && shell.cd(cdAppPath);
		shell.exec('convert' + scale + ' ' + fuzz + ' -define png:exclude-chunks=date "' + file.originalPath + '" -bordercolor transparent -border 1 -trim "' + file.path + '"', next);
	}, callback);
};
...
exports.getImagesSizes = function (files, options, callback) {
  ...
  cdAppPath && shell.cd(cdAppPath);
	shell.exec('identify ' + filePaths.join(' '), function (err, stdout) {
...
exports.generateImage = function (files, options, callback) {
  cdAppPath && shell.cd(cdAppPath);
	...
	shell.exec(command.join(' '), function (err) {
		if (err) return callback(err);
