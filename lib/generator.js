...
var cdAppPath = global.imagemagickPath ? 'cd ' + global.imagemagickPath + ' && ' : '';
...
exports.trimImages = function (files, options, callback) {
	...
	async.eachSeries(files, function (file, next) {
		...
		exec(cdAppPath + 'convert' + scale + ' ' + fuzz + ' -define png:exclude-chunks=date "' + file.originalPath + '" -bordercolor transparent -border 1 -trim "' + file.path + '"', next);
	}, callback);
};
...
exports.getImagesSizes = function (files, options, callback) {
	...
	exec(cdAppPath + 'identify ' + filePaths.join(' '), function (err, stdout) {
...
exports.generateImage = function (files, options, callback) {
	var command = [cdAppPath + 'convert -define png:exclude-chunks=date -quality 0% -size ' + options.width + 'x' + options.height + ' xc:none'];
